...
import type { Job, JobseekerProfile, MatchScoreResult } from "@/types";
...
export default function JobsPage() {
  ...
  useEffect(() => {
    if (session?.role === "jobseeker") {
      (async () => {
        const p = await getJobseekerProfile(session.userId);
        setProfile(p);
      })();
    } else {
      setProfile(null);
  ...
  const filtered = useMemo(() => {
    ...
    list = [...list].sort((a, b) => {
      if (sort === "score") return (b.matchResult?.score ?? -1) - (a.matchResult?.score ?? -1);
