-- Supabase 스키마 설정 (IF NOT EXISTS & 안전한 인덱스 버전)
...
DROP INDEX IF EXISTS idx_self_intros_user_id;
DROP INDEX IF EXISTS idx_self_intros_job_id;
DROP INDEX IF EXISTS idx_scouts_company_id;
DROP INDEX IF EXISTS idx_scouts_jobseeker_id;
DROP INDEX IF EXISTS idx_viewed_candidates_company_id;
...
-- 기존 RLS 정책 삭제 (안전)
DROP POLICY IF EXISTS "Users can view own jobseeker profile" ON jobseeker_profile;
DROP POLICY IF EXISTS "Users can update own jobseeker profile" ON jobseeker_profile;
DROP POLICY IF EXISTS "Users can insert own jobseeker profile" ON jobseeker_profile;
...
DROP POLICY IF EXISTS "Users can view own self intros" ON self_intros;
DROP POLICY IF EXISTS "Companies can view shared self intros" ON self_intros;
DROP POLICY IF EXISTS "Users can insert own self intros" ON self_intros;
DROP POLICY IF EXISTS "Users can update own self intros" ON self_intros;
DROP POLICY IF EXISTS "Users can view own feedback" ON feedback_results;
DROP POLICY IF EXISTS "Users can view relevant scouts" ON scouts;
DROP POLICY IF EXISTS "Companies can send scouts" ON scouts;
DROP POLICY IF EXISTS "Users can update relevant scouts" ON scouts;
DROP POLICY IF EXISTS "Companies can view own viewed candidates" ON viewed_candidates;
DROP POLICY IF EXISTS "Companies can record viewed candidates" ON viewed_candidates;
...
CREATE INDEX IF NOT EXISTS idx_self_intros_user_id ON self_intros(user_id);
CREATE INDEX IF NOT EXISTS idx_self_intros_job_id ON self_intros(job_id);
CREATE INDEX IF NOT EXISTS idx_scouts_company_id ON scouts(company_id);
CREATE INDEX IF NOT EXISTS idx_scouts_jobseeker_id ON scouts(jobseeker_id);
CREATE INDEX IF NOT EXISTS idx_viewed_candidates_company_id ON viewed_candidates(company_id);
